...
export class MonthTaskComponent implements OnInit {
  ...
  public removeEmptyControls(): void {
    ...
    if (formArrayLength > formArray.length) {
      this.dataSource = new MatTableDataSource((this.formSave.get('data') as FormArray).controls);
    }
  }
              ...
              .setControl(
                ...
                new FormGroup({
                  isSelected: new FormControl({
                    value: this.taskProgress[j].progress[parseInt(progressDays[i])].isSelected,
                    disabled: this.currentDay === parseInt(progressDays[i]) ? false : true
                  })
                }))
          }
          this.habits.push(group)
